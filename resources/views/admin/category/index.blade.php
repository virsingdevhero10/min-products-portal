                    ...
                    <div class="card recent-sales overflow-auto">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-borderless datatable" id="category_table">
                                ...
                                <tbody>
                                    @if (!empty($categoryArr) && count($categoryArr) > 0)
                                        @foreach ($categoryArr as $key => $value)
                                            <tr>
                                                <td>#{{ $key + 1 }}</td>
                                                <td>
                                                    @if ($value->cate_picture != '' && file_exists(public_path('categories_images/' . $value->cate_picture)))
                                                        <img src="{{ asset('categories_images/' . $value->cate_picture) }}"
                                                            style="height: 70px; width: 70px;" alt="Cate Picture"
                                                            class="img-profile rounded-circle" />
                                                    @else
                                                        <img src="{{ asset('categories_images/placeholder.svg') }}"
    ...
    <div class="modal fade" id="categoryDeleteModel" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin.category.delete') }}" id="form_delete">
                    @csrf
